...
Class CustomerReward
{
    ...
    public static function loadByCustomerId($customerId, $start = 0, $limit = 0)
    {
        $customerId = intval($customerId);
        $start      = intval($start);
        $limit      = intval($limit);

        $objCustomerRewardArr = NULL;

        $table      = "customer_reward CR
                       LEFT JOIN customer C USING(customer_id)
                    ";
        $condition  = "AND customer_id=" . $customerId;
        $fields     = "CR.*, C.firstname, C.lastname";
    ...
    public static function loadAllInfoCustomerId( $customerId=0 )
    {
        $customerId  = intval($customerId);
        ...
        return $objCustomerRewardArr;
	     ...
	     /**
     * get data from database table customer_reward by cusotmer_id
     *
     * @return CustomerReward
     *
     */
public static function fetchByCustomerId( $customerId )
    ...
    {
        $customerId = intval($customerId);
        return Connection::delData("customer_reward", " AND customer_reward_id=" . $customerId);
    }

    /**
     * get total customer reward point by customer id
     *
     * @param $customerId
     * @return bool
     */
    public static function  getTotalCustomerReward($customerId)
    {
        $table = "customer_reward";
        $condition = "AND customer_id=" . $customerId;
    ...
    /**
     * get total customer reward points
     *
     * @param $customerId
     * @return mixed
     */
    public static function getTotalCustomerRewardBalance($customerId)
    {
        $customerId = intval($customerId);
        $query      = "Select sum(points) as total_points from customer_reward where customer_id = " . $customerId;
